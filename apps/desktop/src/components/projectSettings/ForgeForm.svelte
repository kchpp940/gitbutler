<script lang="ts">
	...
	const selectedOption = $derived(projectDraftStore.draft.forgeOverride || "default");
	...
	function handleSelectionChange(selectedOption: ForgeName) {
		projectDraftStore.updateDraft(projectId, { forgeOverride: selectedOption });
	}

	function updatePreferredGitHubAccount(projectId: string, account: GithubAccountIdentifier) {
		projectDraftStore.updateForgeSettings(projectId, {
			preferredForgeProvider: "github",
			preferredForgeAccount: account,
		});
	}

	function updatePreferredGitLabAccount(projectId: string, account: GitlabAccountIdentifier) {
		projectDraftStore.updateForgeSettings(projectId, {
			preferredForgeProvider: "gitlab",
			preferredForgeAccount: account,
		});
	}
</script>
